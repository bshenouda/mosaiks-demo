...
In this step, we create a grid of polygon cells, and constrain it by the bounding box of the label data. The specified resolution represents the desired resolution at which predictions are later made.

The grid cells allow us to see sum the total length of road segments that fall within each cell, and generate in a next step a tabular dataset of lon/lat coordinates with a corresponding value equal to the sum of all road lengths within that cell.
...
grid_cells <- st_make_grid(label_shp_reprojected, 
        ...
        offset = c(round(label_bbox[1], 3), round(label_bbox[2], 3))) # starting at the min x and y of the bbox rounded to 3 digit precision of the MOSAIKS grid
...
## 3. Spatially join the grid with label shapefile and sum lengths of road in each cell (at the .01 resolution, this takes a bit of time)
