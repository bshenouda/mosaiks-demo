...
label_df <- st_join(grid_sf, label_shp_reprojected[,c("ErosPr","DPA_CANTON","DPA_PROVIN")], join = st_within, left = FALSE) # spatial join grid over shapefile, "ErosPr" is the desired attribute column of the example dataset
...
X_train <- as.matrix(subset(train, select = -c(get(label_value),lat,lon,DPA_PROVIN,DPA_CANTON)))
y_train <- train[,label_value]

X_test <- as.matrix(subset(test, select = -c(get(label_value),lat,lon,DPA_PROVIN,DPA_CANTON)))
