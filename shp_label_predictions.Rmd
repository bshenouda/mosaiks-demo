...
5.  Merge the label dataset with the MOSAIKS features obtained through the File Query
6.  Train data
7.  Make predictions
...
```{r}
plot(label_shp[,"ErosPr"], axes = T) # Plot variable "soil erosion index" ErosPr
```
...
ggplot() +
  ...
  geom_sf(data = grid_square, fill = 'transparent', lwd = 0.1) +
  coord_sf(datum = 4326)  +
  labs(x = "") +
  labs(y = "")
...
ggplot() +
  ...
  geom_sf(data = grid_square, fill = 'transparent', lwd = 0.01) +
  coord_sf(datum = 4326)  +
  labs(x = "") +
  labs(y = "")
...
grid_plot <- ggplot() +
  geom_sf(data = label_df, aes(color = ErosPr), size = 0.5) +
  scale_color_gradientn(colors = sf.colors()) +
  ggtitle("Gridded points")

grid.arrange(shp_plot, grid_plot, ncol = 2)
```
...
```{r}
# max_observed <- max(map_plot$observed
#plot observed 
# observed <- 
  ggplot(data = map_plot, aes(x=lon, y=lat)) +
           geom_point(aes(color= observed), size = 3, alpha = 0.5) + 
           scale_color_distiller(palette = "Spectral", limits = c(0, max_observed)) +
...
#grid.arrange(observed, predicted, ncol = 2)
```
